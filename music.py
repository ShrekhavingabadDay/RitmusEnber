...
yt_dlp_download_format_options = {
...
class MusicPlayer(commands.Cog, name='Music'):
    ...
    async def queue(self, msg, song):
        ...
        if not title1:
            return await msg.send("NEM\nhttps://i.ytimg.com/vi/Vg4NJ_oyyL0/maxresdefault.jpg")
        ...
        if data['queue']:
            ...
            return await msg.send(f"Lejátszási lista {data['title']} hozzáadva a lejátszási sorhoz")
        self.player[msg.guild.id]['queue'].append(
            {'title': title, 'author': msg})
        return await msg.send(f"**{title} hozzáadva a lejátszási sorhoz**".title())
    ...
    async def start_song(self, msg, song):
        new_opts = yt_dlp_format_options.copy()
        ...
        emb = discord.Embed(colour=self.random_color, title='Most szól',
    ...
    async def before_play(self, msg):
        ...
        if msg.author.voice is None:
            return await msg.send('**Csatlakozz egy hangcsatornához, hogy zenét játszhass le!**'.title())
        ...
        if msg.voice_client.channel != msg.author.voice.channel:
            ...
            if self.player[msg.guild.id]['queue']:
                ...
                return await msg.send("Csatlakozz a bottal egy hangcsatornára, hogy zenét tudj adni a lejátszási listához!")
    ...
    async def repeat(self, msg):
        ...
        if msg.guild.id in self.player:
            ...
            return await msg.send("Nem szól zene")
        return await msg.send("Ajjaj")
    ...
    async def reset(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send(f"**{msg.author.display_name}, a bot nem vereti a zenét.**")

        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send(f"**{msg.author.display_name}, menj egy hangcsatornára a bottal!.**")

        if self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Nincs most játszott audió, vagy zene a lejátszási listában**".title(), delete_after=25)
    ...
    async def skip(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("**Nem szól a zene!**".title(), delete_after=60)

        if msg.author.voice is None or msg.author.voice.channel != msg.voice_client.channel:
            return await msg.send("Csatlakozz a bottal egy hangcsatornára!")

        if not self.player[msg.guild.id]['queue'] and msg.voice_client.is_playing() is False:
            return await msg.send("**Nincsenek a listában átugorható számok!**".title(), delete_after=60)
    ...
    async def stop(self, msg):
        ...
        """
        if msg.voice_client is None:
            return await msg.send("A bot nincs hangcsatornához csatlakoztatva!")

        if msg.author.voice is None:
            return await msg.send("Egy hangcsatornán kell lenned a bottal!")
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            ...
            return await msg.send(f"**{msg.author.display_name}, nincs szám a lejátszási listában!**")
    ...
    async def leave(self, msg):
        ...
        if msg.author.voice is None:
            return await msg.send("Egy hangcsatornán kell lenned a bottal, hogy lecsatlakoztathasd!")
    ...
    async def pause(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if msg.voice_client.is_paused() is True:
                return await msg.send("Már megállt a zene!")
    ...
    async def resume(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            if msg.voice_client.is_paused() is False:
                return await msg.send("Már elindult a zene!")
    ...
    async def _queue(self, msg):
        ...
        if msg.voice_client is not None:
            if msg.guild.id in self.player:
                if self.player[msg.guild.id]['queue']:
                    emb = discord.Embed(
                    colour=self.random_color, title='Lejátszási lista:')
        ...
        return await msg.send("Nincsenek zenék a lejátszási listán")
    ...
    async def song_info(self, msg):
        ...
        if msg.voice_client is not None and msg.voice_client.is_playing() is True:
            emb = discord.Embed(colour=self.random_color, title='Most játszott:',
        ...
        return await msg.send(f"**Nincs lejátszandó szám**".title(), delete_after=30)
    ...
    async def join_error(self, msg, error):
        ...
        if error.args[0] == 'Command raised an exception: Exception: playing':
            return await msg.send("**Csatlakozz a bot hangcsatornájához, hogy zenét adhass a lejátszási listához**".title())
    ...
    async def volume(self, msg, vol: int):
        ...
        return await msg.send("**A hangerő beállításához csatlakozz a bot hangcsatornájához!**".title(), delete_after=30)
